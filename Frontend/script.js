...
const API_URL = (configuredApiUrl || (window.location.protocol === "file:" ? "http://127.0.0.1:8000" : window.location.origin)).replace(/\/$/, "");
...
async function startInterview() {
    ...
    if (!questionBanks[type]?.[difficulty] || !Number.isInteger(totalQuestions) || totalQuestions < 1) {
        if (startButton) startButton.disabled = false;
        alert("Invalid interview configuration. Please refresh and try again.");
        return;
    }
    ...
    if (storedCandidateId && storedCandidateId !== candidateId.trim().toLowerCase()) {
        sessionAccessToken = "";
        candidateAccessToken = "";
        sessionStorage.removeItem("interviewSessionToken");
        localStorage.removeItem("interviewCandidateToken");
        localStorage.removeItem("interviewCandidate");
        storedCandidateId = "";
    }
    ...
    try {
        const sessionPayload = await fetchJson(`${API_URL}/session/start`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                session_id: sessionId,
                candidate_id: candidateId,
                ...(candidateAccessToken ? { access_token: candidateAccessToken } : {})
            })
        });
        if (typeof sessionPayload.session_token !== "string" || !sessionPayload.session_token ||
            typeof sessionPayload.candidate_access_token !== "string" || !sessionPayload.candidate_access_token ||
            typeof sessionPayload.candidate_id !== "string" || !sessionPayload.candidate_id) {
    ...
    const pool = [...questionBanks[type][difficulty]];
    for (let i = pool.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [pool[i], pool[j]] = [pool[j], pool[i]];
    }
    questions = pool.slice(0, Math.min(totalQuestions, pool.length));
...
function showQuestion() {
    ...
    currentEvaluationId = createSessionId();
}
...
async function checkAnswer() {
    ...
    try {
        const payload = await fetchJson(`${API_URL}/evaluate`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-Session-Token": sessionAccessToken,
                "X-Evaluation-Id": currentEvaluationId
            },
            body: JSON.stringify({
                session_id: sessionId,
                candidate_id: candidateId,
...
async function finishInterview() {
    ...
    try {
        const summary = await fetchJson(`${API_URL}/history/candidate/${encodeURIComponent(candidateId)}/summary`, { headers: { "X-Session-Token": candidateAccessToken } }, 10000);

...
function finishInterview() {
    ...
    const technicalAccuracy = average(
        interviewScores.map(item => Number(item.technical_accuracy || 0))
    );

    const relevance = average(
        interviewScores.map(item => Number(item.relevance || 0))
    );

    const clarity = average(
        interviewScores.map(item => Number(item.clarity || 0))
    );

    const completeness = average(
        interviewScores.map(item => Number(item.completeness || 0))
    );
    ...
    document.getElementById("interview").innerHTML = `
        ...
        <p>
            ...
            ${interviewScores.length}/${totalQuestions}
        </p>
        ...
        <p>
            Keep practicing and focus on giving detailed explanations

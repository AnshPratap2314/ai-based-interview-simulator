...
app = FastAPI(title="AI Interview Preparation OS", version="0.4.0")
...
# Lightweight per-process abuse protection. For multi-instance production deployments,
# move this to a shared store such as Redis/API gateway rate limiting.
...
def start_session(data: dict):
    """Create an authenticated interview session.

    The returned access token is a bearer credential for this candidate. It is
    never stored in the database in plaintext; only its hash is persisted.
    """
    candidate_id = str(data.get("candidate_id", "")).strip()
    session_id = str(data.get("session_id", "")).strip()
    access_token = data.get("access_token")
    if not candidate_id or len(candidate_id) > 150:
        raise HTTPException(status_code=400, detail="A valid candidate ID is required.")
    if not session_id or len(session_id) > 100:
        raise HTTPException(status_code=400, detail="A valid session ID is required.")

    try:
        token, is_new = ensure_candidate_access(candidate_id, access_token)
    ...
    session_token = token
    try:
        create_session(session_id, candidate_id, session_token)
    except sqlite3.IntegrityError:
        raise HTTPException(status_code=409, detail="Session ID already exists. Start a new interview.")
    return {
...
def evaluate_answer(request: Request, data: AnswerRequest, x_session_token: str | None = Header(default=None)):
    if not check_rate_limit(f"evaluate:{request.client.host if request.client else 'unknown'}"):
        raise HTTPException(status_code=429, detail="Too many evaluation requests. Please wait and try again.")
    if not x_session_token or not data.session_id or not data.candidate_id:
    ...
    try:
        evaluation = evaluate_with_retry(client, data, MODEL_NAME)
        latency_ms = round((time.perf_counter() - started) * 1000)

        try:
            save_evaluation(data, evaluation)
        except Exception:
            # Persistence failing should never break the response the user is waiting on.
            logger.exception(
...
def history(session_id: str, x_session_token: str | None = Header(default=None)):
    """Return only the history belonging to the authenticated session."""
...
def candidate_history(candidate_id: str, x_session_token: str | None = Header(default=None)):
    """Return candidate history only to the candidate's bearer credential."""
...
def candidate_summary(candidate_id: str, x_session_token: str | None = Header(default=None)):
    """Return candidate trend data only to the candidate's bearer credential."""
    if not x_session_token or not authorize_candidate(candidate_id, x_session_token):
        raise HTTPException(status_code=403, detail="Candidate access denied.")
